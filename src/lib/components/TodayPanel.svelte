...
<section class="today" aria-label={`Today’s saved usage · ${projectName}`}>
  <div class="lead">
...
<style>
  ...
  .kicker { display: flex; align-items: center; gap: 8px; color: var(--ink-dim); font: 600 11px/1.5 var(--font-data); letter-spacing: 1.2px; text-transform: uppercase; }
  .dot { flex-shrink: 0; width: 7px; height: 7px; border-radius: 50%; background: var(--brand); box-shadow: 0 0 8px var(--brand); }
  .project { margin: 12px 0 0; color: var(--brand); font: 600 13px var(--font-ui); overflow-wrap: anywhere; }
  .headline { display: flex; align-items: baseline; flex-wrap: wrap; gap: 6px 12px; margin: 10px 0 0; }
  .number { font: 700 44px/1 var(--font-display); letter-spacing: -2px; color: var(--ink); font-variant-numeric: tabular-nums; }
