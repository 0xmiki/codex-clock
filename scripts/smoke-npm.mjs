...
assert(packed.files.every(file => file.path === 'package.json' || file.path === 'README.md' || file.path === 'bin/codex-watch.cjs' || file.path === 'dist/server.mjs' || file.path.startsWith('dist/web/')));
runNpm(['install', '--prefix', root, '--no-audit', '--no-fund', join(root, packed.filename)]);
const installed = join(root, 'node_modules/codex-watch');
const cli = join(installed, 'bin/codex-watch.cjs');
