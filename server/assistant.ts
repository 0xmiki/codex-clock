...
export function usagePrompt(question: string, snapshot: Snapshot, now = Date.now(), transcript?: string | null, selectedThreadId?: string | null) {
  ...
  return `${tierContext}\n\nCurrent saved usage snapshot. "Today" means tokens recorded today (UTC), including resent context. All row token counters cover today only. Daily model costs and turn counts are unavailable; do not infer them from lifetime totals. Usage Scores compare average estimated cost per call over the last 5 calls with the median cost per call of up to 30 other recent threads on the SAME model. Exclude the graded thread. Each peer contributes one per-call average regardless of lifetime call count. normalCost is that same-model median; costRatio is the midpoint of recentCost divided by normalCost and determines both the letter grade and displayed multiplier. At least 5 peers are required. A <=1x usual, B <=1.5x, C <=2x, D <=3x, F >3x. An F means unusually high estimated cost per call for that model, not proven waste. Larger context, less caching, more output and tier premiums can raise consumption. The score measures cost per call, not per token, per minute or total task cost. A consistently expensive personal baseline can still produce an A. These are API-equivalent estimates, not exact subscription-limit consumption.\n${JSON.stringify(rows)}${selected ? `\nSelected thread: ${JSON.stringify({ id: selected.id, title: selected.title, efficiency: grades.get(selected.id) })}` : ''}${transcript ? `\n\nSelected thread transcript (tail):\n${transcript}` : ''}\n\nUser question: ${question}`;
}
