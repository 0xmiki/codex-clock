    ...
    refreshing = (async () => {
      ...
      try {
        ...
        do {
          ...
          for (const thread of page.data) if (!listed.has(thread.id)) listed.set(thread.id, thread);
        ...
        const threads: Thread[] = await mapConcurrent([...listed.values()], 8, async t => {
        ...
        paths.clear();
        for (const t of listed.values()) if (t.path) paths.set(t.id, t.path);
      ...
      finally { rpc.close(); progress.phase = 'idle'; }
    })().finally(() => { refreshing = undefined; });
    return refreshing;
  }
  return { state, refresh, progress, indexStats: readUsage.stats, inspectThread: async (reference: string) => {
